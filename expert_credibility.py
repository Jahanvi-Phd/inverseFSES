...
for _, row in data.iterrows():
    ...
    credibility = expert_credibility[expert]
    opinion = np.random.choice(O, p=[1-credibility, credibility])
...
agree_df.to_excel(os.path.join(output_dir, f"Agree_IFSES_{timestamp}.xlsx"), index=False)
disagree_df.to_excel(os.path.join(output_dir, f"Disagree_IFSES_{timestamp}.xlsx"), index=False)
...
agree_norm.to_excel(os.path.join(output_dir, f"Agree_Normalized_{timestamp}.xlsx"), index=False)
disagree_norm.to_excel(os.path.join(output_dir, f"Disagree_Normalized_{timestamp}.xlsx"), index=False)
...
agree_norm.to_excel(os.path.join(output_dir, f"Agree_Weighted_{timestamp}.xlsx"), index=False)
disagree_norm.to_excel(os.path.join(output_dir, f"Disagree_Weighted_{timestamp}.xlsx"), index=False)
...
net_scores.to_excel(os.path.join(output_dir, f"Drug_Ranking_{timestamp}.xlsx"), index=False)
